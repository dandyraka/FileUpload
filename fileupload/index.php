  ...
  if(!empty($_FILES['uploaded_file']))
  {
	...
	if(in_array($ext, $allowed)){
		if(move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $path)) {
		  ...
		  $shareurl = "http://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]download.php?filename=$namafile";
		  ...
		  if($ext == "txt"){
			  echo "<br>Ubah data berkas Anda <a href='edit.php?filename=".$filename."' target='_blank'><b>DISINI</b></a>";
		  }
		  echo "<br>Share link <input type='text' class='form-group' value='".$shareurl."'>";
		  echo "</div>";
		  
		  $open = fopen('log.txt', 'a');
			fwrite($open, date('Y/m/d').' '.$filename.' - diunggah'.PHP_EOL);
			fclose($open);
		} else{
